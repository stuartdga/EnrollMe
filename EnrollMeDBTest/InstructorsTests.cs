...
namespace EnrollMeDBTest
{
    ...
    public class InstructorsTests
    {
        ...
        private string _lastName = Helper.GenerateId();

        [TestMethod]
        public void Instructors_AddRemove()
        {
            var instructor = controller.Add("asdf", "", _lastName);
            Assert.IsTrue(instructor.InstructorId > 0);
            var instructor2 = controller.Add("asdf", "", _lastName);
            Assert.AreEqual(instructor, instructor2);
            var result = controller.Remove("asdf", "", _lastName);
            Assert.AreEqual(result, 1);
            result = controller.Remove("asdf", "", _lastName);
        ...
        {
            controller.Add("asdf", "", _lastName);
            var instructors = controller.GetAll();
            int i = instructors.Count();
            Assert.IsTrue(i > 0);
            controller.Remove("asdf", "", _lastName);
        }
